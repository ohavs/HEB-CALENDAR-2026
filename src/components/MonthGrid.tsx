...
export function WeekdayHeader({ weekStart }: { weekStart: 0 | 1 }) {
  ...
  return (
    <div className="grid shrink-0 grid-cols-7 px-1.5 pb-1.5">
      {names.map((name, i) => (
        <span
          key={name}
...
export function MonthGrid({
  ...
  const capacity =
    rowHeight > 0 ? Math.max(1, Math.min(3, Math.floor((rowHeight - 44) / 13))) : 2;

  return (
    <div
      ref={ref}
      className="mb-auto grid max-h-[600px] min-h-0 flex-1 grid-cols-7 grid-rows-6 gap-[1px] overflow-hidden px-1.5"
    >
